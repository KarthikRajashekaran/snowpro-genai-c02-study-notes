...
> 🔎 **C02 alignment:** This scenario teaches the RAG *pattern* from first principles (manual chunk → embed → vector search → generate). For the C02 exam, prefer the managed path: **`AI_PARSE_DOCUMENT`** to parse, **`SPLIT_TEXT_RECURSIVE_CHARACTER`** to chunk, **Cortex Search** for retrieval + reranking (see [1.4](../1_Snowflake_for_Gen_AI_Overview/1.4_Cortex_Search_Analyst_Semantic_Views.md)), and **`AI_COMPLETE`** to generate. Legacy `SNOWFLAKE.CORTEX.*` names below remain conceptually valid but map to the `AI_*` family covered in [Domain 2](../2_Snowflake_Gen_AI_and_LLM_Functions/README.md).
