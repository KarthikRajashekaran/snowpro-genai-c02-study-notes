[< Cortex AI Features](./1.2_Snowflake_Cortex_AI_Features.md) | **1.3 Vector Data Types & Operations** | [1.4 Cortex Search & Analyst >](./1.4_Cortex_Search_Analyst_Semantic_Views.md)
***
...
> **Scope note (C02):** In Domain 1 you only need the *conceptual* picture of the `VECTOR` type and how embeddings are stored and compared. The full catalog of vector functions — `VECTOR_COSINE_SIMILARITY`, `VECTOR_L2_DISTANCE`, `VECTOR_INNER_PRODUCT`, `EMBED_TEXT_*`, etc., with exact signatures — is exam **Domain 2** material. See [Domain 2: Snowflake Gen AI & LLM Functions](../2_Snowflake_Gen_AI_and_LLM_Functions/README.md). Examples below are illustrative and cross-referenced there.
...
Snowflake does not use manually created secondary indexes on `VECTOR` columns the way traditional B-tree indexes work. For production-scale similarity search and RAG, the recommended path is the managed **Cortex Search** service (see [1.4](./1.4_Cortex_Search_Analyst_Semantic_Views.md)), which handles chunking, embedding, indexing, and hybrid retrieval for you. For do-it-yourself vector search, you compute similarity directly with the vector functions and can reduce scanned data with pre-filters and pruning.

```sql
-- DIY similarity search: filter on scalar columns first, then rank by vector similarity
SELECT id, document_text,
       VECTOR_COSINE_SIMILARITY(embedding, :query_embedding) AS score
FROM document_embeddings
WHERE category = 'technical_docs'      -- prune with ordinary predicates
ORDER BY score DESC
LIMIT 10;
```
